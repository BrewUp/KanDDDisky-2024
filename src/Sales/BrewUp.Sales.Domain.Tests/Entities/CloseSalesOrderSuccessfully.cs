...
public class CloseSalesOrderSuccessfully : CommandSpecification<CloseSalesOrder>
{
    ...
    protected override CloseSalesOrder When()
    {
        return new CloseSalesOrder(_salesOrderId, _correlationId, _salesOrderNumber);
    }

    protected override ICommandHandlerAsync<CloseSalesOrder> OnHandler()
    {
        return new CloseSalesOrderCommandHandler(Repository, new NullLoggerFactory());
    }

    protected override IEnumerable<DomainEvent> Expect()
    {
        yield return new SalesOrderClosed(_salesOrderId, _correlationId);
    }
}